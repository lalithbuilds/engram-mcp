...
def benchmark_fts5_vs_like():
    ...
    start = time.time()
    for _ in range(1000):
        conn.execute("SELECT * FROM memories_fts WHERE memories_fts MATCH 'memory' LIMIT 5").fetchall()
    fts5_time = time.time() - start
    
    start = time.time()
    for _ in range(1000):
        conn.execute("SELECT * FROM memories WHERE content LIKE '%memory%' LIMIT 5").fetchall()
...
def benchmark_auto_decay():
    ...
    for i in range(1, 11):
        ...
        conn.execute(
            "INSERT INTO memories (id, category, content, importance, created_at, updated_at, access_count, last_accessed_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?)",
            (mid, "decay-test", f"Memory {i}", i, server.now(), server.now(), server.now())
        )
    
    conn.commit()
    
    initial = conn.execute("SELECT id, importance FROM memories WHERE category='decay-test' ORDER BY id").fetchall()
    ...
    decayed = conn.execute("SELECT id, importance FROM memories WHERE category='decay-test' ORDER BY id").fetchall()
...
def benchmark_concurrent_reads():
    ...
    for i in range(100):
        ...
        conn.execute(
            "INSERT INTO memories (id, category, content, importance, created_at, updated_at, access_count, last_accessed_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?)",
            (mid, "concurrent", f"Memory {i}", 5, server.now(), server.now(), server.now())
        )
    conn.commit()
    conn.close()
    ...
    def concurrent_reader():
        c = server.get_db()
        for _ in range(100):
            c.execute("SELECT COUNT(*) FROM memories WHERE category='concurrent'").fetchone()
            read_count[0] += 1
        c.close()
